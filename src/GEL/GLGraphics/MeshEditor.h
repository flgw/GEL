...
namespace GLGraphics {
    ...
    private:
        ...
        std::shared_ptr<Console> theConsole;
        ...
        std::shared_ptr<std::thread> m_sckThrd;

    public:

        MeshEditor(std::shared_ptr<Console> cs):theConsole(cs),
        brush_type("smooth"),
        selection_mode(0),
        active(0),
        ...
        display_gamma(2.2)
        {
            m_sckThrd = std::make_shared<std::thread>(SocketConnector::listen_commands);
        }

        MeshEditor():MeshEditor(std::shared_ptr<Console>(new Console)){}
